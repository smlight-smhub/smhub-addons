...
> Since the Docker container runs in isolation, PlatformIO will not be able to pull packages from private repositories (like `platform-sg2000` or `framework-sg2000-rtos`) unless git credentials or SSH keys are explicitly forwarded into the container. For private repos, we strongly recommend **Method 2**.
